...
-- 1 is script enabled | 0 script is disabled
SetConvar("bzk_carspawn_enable", "1")

RegisterCommand('spawn_car', function(source, args)
    -- Return if the script is disabled
    if(GetConvarInt("bzk_carspawn_enable", 1) == 0) then return end 
    ...
    then
        ...
        local car_string = LoadResourceFile("myfirstmode", "carnames.txt")
        local car_names = {}
        ...
        then 
            TriggerEvent('chat:addMessage', {
                args = {'~p~[CarSpawner] ~r~ERROR ~w~The entered model '.. vehicleName .. ' does not exist.'}
            })
            return
        end
    end
    ...
    local vehicle = CreateVehicle(vehicleName, position.x, position.y, position.z, GetEntityHeading(playerPed), true, false)
    ...
    TriggerEvent('chat:addMessage', {
        args = {'~p~[CarSpawner] ~g~Spawned a new ' .. vehicleName .. '!!'}
    })
end, false)